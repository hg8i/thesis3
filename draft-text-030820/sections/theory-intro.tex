...
The SM describes the matter particles at the most fundamental level -- no internal structures that appear as mathematical point-like objects within the theory.
Particles with integer spins (0, 1, 2) are classified as Bosons, while particles with half-integer spins (\half) are classified as Fermions in the theory.
...
% Bosons
The vector bosons mediate forces between the particles.
The most familiar boson, the photon, mediates the electromagnetic force between charged particles.
The gluons carry the strong nuclear force, bounding the quarks inside hadrons.
The \W and \Z bosons carry the weak nuclear force, responsible for nuclear beta decays.
Finally, the recently discovered Higgs boson mediates a momentum exchange between particles to generate masses of the particles.
% Summary
These particles are summarized in the SM shown in Table \ref{tab:particles}.
...
Enormous energy concentrations are required to enable the production of massive particles and facilitate interactions with small cross sections.
In order to study these processes, it is convenient to define units of measurements using energy as 
the basic unit to describe
the particle mass, momentum, life-time, and their flight distance (length).
...
The TeV scale was first reached at the Tevatron  proton-antiproton collider at the Fermilab in US in 80th of last Century. The energy at the 
Large Hadron Collider at CERN has been further increased to 13 TeV at 2015.
...
In standard international (SI) units, energy is measured in units of joules $[J]$ that equals to [kg$\frac{\text m^2}{\text s^2}$]. Mass is, therefore, expressible as energy divided by velocity squared.
...
In cases where their presence can be inferred from the quantity, the constants $\hbar$ and $c$ are often taken as 1 in \emph{Ntural units}, leaving powers of GeV in all the measurement quantities.
...
Likewise, the probability of a particular interaction between particles is measured in units of area, called as cross-section.
The unit \emph{barn} is defined such that 1b=$10^{-24}\text ~cm^2$.
...
As the progenitors remark, the barn is quite a large area to describe particle interactions. Therefore prefixed versions like picobarn ($1\text{pb}=10^{-12}$b) and femptobarn ($1\text{fb}=10^{-15}$b) are commonly used in colliding beam experiments.
