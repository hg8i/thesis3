...
Looking at the upper left hand $2\times2$ block of $\lambda^1$, $\lambda^2$, and $\lambda^3$ reveals that they are identical to the Pauli matrices in Equation \ref{eqn:pauli}.
...
A \emph{Lagrangian density}, often shortened to a \emph{Lagrangian}, is a mathematical representation of a physical system.
They have an explicit form that depends on the system that they represent.
...
This section will identify and describe Lagrangian in particle physics.

\subsubsection{Fields}

In Quantum Field Theory (QFT), operators are introduced for each point in spacetime $x^\mu$.
These \emph{operators}, $\phi$, are labeled by their corresponding point as $\phi(x)$.
...
\begin{tabular}{l | l l l}
\toprule
Field & Generic Symbol & Transformation \\
Dirac & $\Psi(x)$ & \\
Weyl & $\psi(x)$ & \\
Vector & $A(x)$ & \\
Scalar & $\phi(x)$ & \\
