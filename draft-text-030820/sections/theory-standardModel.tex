...
In QED, the photon gauge field $A_\mu$ was introduced to make the derivative covariant.
...
\begin{equation}\begin{split}
    D_\mu q_f\equiv\partial_\mu q_f+ig_sA_\mu^aT_i^{aj} q_f.
\end{split}\end{equation} 
...
The coupling constant $g_s$ is related to the bare coupling constant $\overline{\alpha}_s=\frac{g^2_s}{4\pi}$, however this isn't the full physical picture.
...
As is the case in QCD, the generators of the groups are associated with bosons that help define covariant derivatives.
...
As was the case in QED, the generator of U(1) is associated with a boson $B_\mu$.
...
First, noting the dependence of the covariant derivative in Equation \ref{eqn:ewCovDeriv}, there are no terms containing right-handed fermions and either $W^1_\mu$ or $W^2_\mu$. Since these form the basis for the \W bosons, this shows that those do not interact with the right-handed component of fermion fields.
...
\end{split}\end{equation} 
The Feynman rules corresponding to these interactions are shown in Figure \ref{fig:ewRules0}.
...
In particular, the neutral current Lagrangian 
...
Instead of the choice of unitary matrix $f_L'=U_Lf_L$ and $f'_R=U_Rf_R$ that is used to define the mass-eigenstates for leptons, an analogous definition for quarks is made: $q'_{L,i}=V_i^jq_{L,j}$.
...
~&~&                        & +\Psi_iy_{ij}\Psi_j\Phi+\text{h.c.}                &\text{Yukawa couplings} &~ \notag\\
