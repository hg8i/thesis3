\subsection{Physics with Lagrangians}\label{sec:smMath}
...
An useful corollary to the prediction of the cross section described in Section \ref{sec:me} is the description of particle decays derived from a model.
...
This process is complicated, but fortunately particular leading terms of the matrix element can often be abstracted and represented graphically with \emph{Feynman diagrams}.
...
The coupling constant in Equation \label{eqn:feynmanRuleInt} therefore appears in the Hamiltonian along with combinatorial factors that cancel the denominator. 
This is in turn is used to calculate the Feynman rule for the interaction term's contribution to the matrix element is $-i\lambda$.
...
The Feynman rule for the contribution of a free term to the matrix element is $i(P^{-1})_{ij}$, where spacial derivatives $\partial_\mu$ are replaced with the particle's momentum $-ip_\mu$.
...
Finally, the fields in the Lagrangian are expanded in term of creation and annihilation operators.
...
These must coefficients for a basis for the equation of motion.
